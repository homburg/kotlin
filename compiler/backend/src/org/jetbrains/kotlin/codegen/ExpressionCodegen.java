...
public class ExpressionCodegen extends KtVisitor<StackValue, StackValue> implements LocalLookup {
    ...
    private CodegenContext getBackingFieldContext(
        ...
        switch (accessorKind) {
            ...
            case IN_COMPANION: return context.findParentContextWithDescriptor(containingDeclaration.getContainingDeclaration());
            case FROM_LITERAL: return context.findParentContextWithDescriptor(containingDeclaration);
    ...
    public StackValue.Property intermediateValueForProperty(
        ...
        boolean isBackingFieldInCompanion = AsmUtil.isPropertyWithBackingFieldInOuterClass(propertyDescriptor);
        if (isBackingFieldInCompanion && forceField) {
            fieldAccessorKind = FieldAccessorKind.IN_COMPANION;
        }
        else if (syntheticField && context.getParentContext().getContextDescriptor() != containingDeclaration) {
            fieldAccessorKind = FieldAccessorKind.FROM_LITERAL;
        }
